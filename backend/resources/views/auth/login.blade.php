<x-auth-shell>
  ...
  @if (($as ?? '') === 'admin')
    <p class="login-sub">Admin sign-in with username and password.</p>
  @else
    <p class="login-sub">Enter your username. We will email a one-time code to the address on your account.</p>
  @endif
  @if (!empty($success))
  ...
  @if (($as ?? '') === 'admin')
    <p class="login-forgot login-forgot--admin"><a href="/login">Sign in with one-time code</a></p>
  @else
    <p class="login-forgot login-forgot--admin"><a href="/login?as=admin">Login as Admin</a></p>
  @endif
</x-auth-shell>
