...
<body class="supervisor-shell executive-console">
  @php
    ...
    $positionLine = $user['position'] ?: 'Executive Committee';
    $stats = $stats ?? [];
    ...
    <nav class="supervisor-sidebar__nav" aria-label="Executive Committee navigation">
      @foreach ($nav as $item)
        <a href="{{ $item['href'] }}"
          ...
          @if (($item['id'] ?? '') === 'overview' && (($stats['total'] ?? 0) > 0))
            <span class="supervisor-sidebar__badge" aria-label="{{ (int) ($stats['total'] ?? 0) }} pending">{{ (int) ($stats['total'] ?? 0) }}</span>
          @endif
        </a>
      @endforeach
    </nav>
  ...
  <div class="supervisor-content">
    <header class="console-topbar" aria-label="Page toolbar">
      ...
      <div class="console-topbar__actions">
        ...
        <form method="post" action="/executive/notifications/read-all" class="notif-panel__mark-all">
          @csrf
