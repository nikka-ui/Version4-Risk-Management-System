...
<body class="supervisor-shell officer-console dept-head-console">
  @php
    ...
    $deptLabel = $user['department'] ?? 'Department';
  ...
  <aside class="supervisor-sidebar">
    ...
    <nav class="supervisor-sidebar__nav" aria-label="Department Head navigation">
      @foreach ($nav as $item)
        @php
          $count = isset($item['statKey']) ? (int) ($stats[$item['statKey']] ?? 0) : 0;
        @endphp
        <a href="{{ $item['href'] }}"
          ...
          @if ($count > 0)
            <span class="supervisor-sidebar__badge{{ !empty($item['critical']) ? ' supervisor-sidebar__badge--critical' : '' }}"
                  aria-label="{{ $count }} pending">{{ $count }}</span>
          @endif
        </a>
      @endforeach
    </nav>
  ...
  <div class="supervisor-content">
    <header class="console-topbar" aria-label="Page toolbar">
      ...
      <div class="console-topbar__actions">
        ...
        <form method="post" action="/dept/notifications/read-all" class="notif-panel__mark-all">
          @csrf
